...
//  Swift 5 going further Copyright (c) 2021 Ionut Sabau. All rights reserved.
//
...
@objc public protocol LPRTableViewDelegate: NSObjectProtocol {
    ...
    @objc optional func shouldReorder(_ tableView: UITableView, rowAt indexPath: IndexPath) -> Bool

    /** Called when the dragging gesture's vertical location changes. */
    @objc optional func finishedReordering(_ tableView: UITableView, from initialSourceIndexPath: IndexPath, to finalDestinationIndexPath: IndexPath)
...
extension LPRTableView: UIGestureRecognizerDelegate {
    ...
    open override func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        ...
        return (rows > 0) && (indexPath != nil) && canMoveRowAt(indexPath: indexPath!) && canReorderRowAt(indexPath: indexPath!)
    }
...
extension LPRTableView {
    ...
    fileprivate func canReorderRowAt(indexPath: IndexPath) -> Bool {
        return self.longPressReorderDelegate?.shouldReorder?(self, rowAt: indexPath) ?? true
    }
    ...
    @objc internal func _longPress(_ gesture: UILongPressGestureRecognizer) {
        ...
        case .began: // Started
            ...
            guard let indexPath: IndexPath = indexPath, var cell: UITableViewCell = cellForRow(at: indexPath) else { break }
            ...
            if draggingView == nil {
                ...
                if let draggingView: UIView = draggingView {
                    ...
                    UIView.animate(withDuration: 0.25) {
                        self.longPressReorderDelegate?.tableView?(self, showDraggingView: draggingView, at: indexPath)
                    }
                    ...
                    UIView.animate(withDuration: 0.25) {
                        ...
                        draggingView.center = CGPoint(x: self.center.x, y: self.newYCenter(for: draggingView, with: location))
                    }
                }
            }
        ...
        case .ended where currentLocationIndexPath != nil: // Dropped
            if let initial = self.initialIndexPath, let current = self.currentLocationIndexPath {
                self.longPressReorderDelegate?.finishedReordering?(self, from: initial, to: current)
            }
            fallthrough
        case .cancelled,
             .failed:
...
open class LPRTableViewController: UITableViewController, LPRTableViewDelegate {
    ...
    /** Called when checking if the row should respond to long press*/
    open func shouldReorder(_ tableView: UITableView, rowAt indexPath: IndexPath) -> Bool {
        // Empty implementation, just to simplify overriding (and to show up in code completion).
        return true
    }

    /** Called when the reordering is finished */
    open func finishedReordering(_ tableView: UITableView, from initialSourceIndexPath: IndexPath, to finalDestinationIndexPath: IndexPath) {
        // Empty implementation, just to simplify overriding (and to show up in code completion).
    }
